...
-- This file creates the necessary tables and functions for comprehensive audit logging
...
BEGIN
    ...
    SELECT (setting_value->>'value')::INTEGER INTO retention_days
    ...
    INSERT INTO audit_logs (
        ...
        resource_type, new_values, metadata
    ) VALUES (
        NULL, 'System', 'system', 'audit_cleanup', 'system',
        'audit_logs', 
...
CREATE TRIGGER update_audit_log_settings_updated_at
    BEFORE UPDATE ON audit_log_settings
    FOR EACH ROW
...
SELECT 
    ...
    CASE 
        WHEN al.old_values IS NOT NULL AND al.new_values IS NOT NULL THEN
            jsonb_build_object(
                'changed_fields', (
                    SELECT jsonb_object_agg(key, jsonb_build_object('old', value, 'new', al.new_values->key))
                    FROM jsonb_each(al.old_values)
                    WHERE al.new_values->key IS DISTINCT FROM value
                )
            )
        ELSE NULL
    END as changes,
